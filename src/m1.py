...
class CircleChanger(object):
    ...
    def get_distance_from(self, point):
        ...
        distance = point.get_distance_from(center)
        return distance
    ...
    def swell_or_shrink_once(self, amount_to_swell_or_shrink):
        ...
        new_thickness = random.randrange(3, 16)
        self.circle.outline_thickness = new_thickness
    ...
    def swallow(self, other_circle_changer):
        ...
        new_radius = self.center.get_distance_from(new_center)
        fill_color = 'red'
        colors = self.colors + other_circle_changer.colors

        new_circlechanger = CircleChanger(new_center.x, new_center.y,
                                          new_radius, fill_color, colors)

        return new_circlechanger
    ...
    def change_to_next_color_in_tuple(self):
        ...
        ################################################################
        if self.num_times_called == len(self.colors):
            self.num_times_called = 0

        self.circle.fill_color = self.colors[self.num_times_called]
